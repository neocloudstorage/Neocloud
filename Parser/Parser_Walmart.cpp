   ...
   if((new string(argv[1]))->compare(*(new string("fetch"))) == 0)          //fetch mode. Fetches the pages for a list of url's and parses
     {
       ...
       while( ifs.good() )
         {
           ifs.getline(line, 1024);                      //reads a block of 1024 characters in loop
           string lineS = line;
	         ...
	         while(stritr != enditr)
        	   {
                 ...
                 count = count + 1;                   //number of links parsed
	 ...
	 while(stritr != enditr)
     ...
     else if((new string(argv[1]))->compare(*(new string("nofetch"))) == 0)      //no fetch mode, parses crawled pages
       {
          ifstream ifs;
          ofstream ofs;
          //string pathToReadFile = "crawledData_Walmart/seg/dump";              //crawled pages
          ...
          while( ifs.good() && read < length)
           {
             ...
             tr1::unordered_map<int,string> productList;                         //parses product name
             parseProductsName(page,productList);
             tr1::unordered_map<int,string> productPrice;                        //parses product price
             parseProductsPrice(page,productPrice);
             tr1::unordered_map<string, string> association;                     //associates name and price
             associateProductNameAndPrice(productList, productPrice, association);
             tr1::unordered_map<string, string>::iterator stritr = association.begin();
             tr1::unordered_map<string, string>::iterator enditr = association.end();
    ...
    while(i < size)
      {
          ...
          i++;
          string link = "<";
          if(i+31 < size && page.substr(i,31).compare("a class=\"prodLink GridItemLink\"")==0 ) //if it is an anchor tag with attribute prodlink
            {
                //copy line until end of anchor tag
                ...
                size_t pos = link.find("href=\"");                 //if the anchor tag has attribute href
                if (pos != string::npos)
                   {
                      ...
                      if (pos != string::npos)
                         {
                           ...
                           if(token.substr(0,4) == "href")
                             url = url + token.substr(6);         //link for the product


                           size_t post = link.find("title=\"");   //products name
                           post = post + 7;
                           string productItem = "";
                           while(post+1<size && ( link[post] !='\"' || link[post+1] != '>' ) )   //until title ends
                             {
          ...
          if(i+3 < size && (page[i] == 's' && page[i+1] == 'p' && page[i+2] == 'a' && page[i+3] == 'n') )
            {
                ...
                size_t pos = link.find("class=\"bigPriceText2\"");  //if the div or span tag has attribute itemprop=price
                if (pos != string::npos)
                   {
                      pos = link.find_last_of(">");
                      if (pos != string::npos)
